...
//    solveQP(L0, c, Aeq, Beq, Aineq, Bineq, res, active, this->get_logger());
    ...
    Eigen::MatrixXd G(7, 7), CE(6, 7), CI(14, 7);
    Eigen::VectorXd g0(7), ce0(6), ci0(14), res(7);
    computeQPMatrices(CE, ce0, CI, ci0);
...
//    std::cout << "ci0: " << ci0 << std::endl;
    ...
    try
    {
        cost = QP::solve_quadprog(G, g0, CE, ce0, CI, ci0, res);
    }
...
//    std::cout << "res: " << res << std::endl;
    ...
    std::vector<double> resv(res.size());
    for (int i = 0; i < (int)res.size(); i++)
        resv[i] = res(i);

    std::vector<double> dfhat = {resv[0], resv[1], resv[2], resv[3]};
    RCLCPP_INFO(this->get_logger(), "f0hat: (%.3f, %.3f, %.3f, %.3f); dfhat: (%.3f, %.3f, %.3f, %.3f); dF: (%.3f, %.3f, %.3f), cost: %.3f",
                f0hat[0], f0hat[1], f0hat[2], f0hat[3], resv[0], resv[1], resv[2], resv[3], resv[4], resv[5], resv[6], cost);
//    f0hat = vpColVector(dfhat);
    f0hat += vpColVector(dfhat);
    alpha0 += resv[4], beta0 += resv[5], gamma0 += resv[6];

    vpColVector tau(f0hat.size());
    for (int i = 0; i < nCables; i++)
    {
        if (f0hat[i] > fMax)
            f0hat[i] = fMax;
        if (f0hat[i] < fMin)
            f0hat[i] = fMin;
        tau[i] = f0hat[i]*lengths[i];
    }
    this->sendTensions(tau);

    RCLCPP_INFO(this->get_logger(), "Tensions: (%.3f, %.3f, %.3f, %.3f); rd: (%.3f, %.3f, %.3f), v: (%.3f, %.3f, %.3f, %.3f, %.3f, %.3f)",
                tau[0], tau[1], tau[2], tau[3], rd[0], rd[1], rd[2], v[0], v[1], v[2], v[3], v[4], v[5]);
